...
openssl req -new -key server.key -out server.csr -subj "/CN=192.168.2.30"
...
openssl pkcs8 -topk8 -outform DER -in ~/ssl/user.key -out ~/ssl/user.key.pk8 -nocrypt
scp ~/ssl/user.pem ~/ssl/user.key ~/ssl/user.key.pk8 ~/ssl/root.pem $USER@192.168.1.20:"$HOME"/
